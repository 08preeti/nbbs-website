"use client";
import { footerData } from "@/data/footer";
import Image from "next/image";
import Link from "next/link";
import { useState } from "react";
import { usePathname } from "next/navigation";

export default function Footer() {
  const [active, setActive] = useState("footerItems");
  const pathname = usePathname();

  const handleLogoClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
    setActive("home");

    if (pathname === "/") {
      e.preventDefault();

      const homeSection = document.getElementById("home");
      const headerOffset = 80;
      const targetTop = homeSection
        ? homeSection.getBoundingClientRect().top + window.scrollY - headerOffset
        : 0;

      window.scrollTo({ top: targetTop, behavior: "smooth" });
    }
  };

  const linkClass =
    "text-body-md w-fit text-slate-300 transition-all duration-200 hover:translate-x-1 hover:text-white";

  return (
    <footer className="w-full border-t border-[#141a32] bg-[#141a32]">
      <div className="container page-x section-y">
        <div className="grid grid-cols-1 gap-10 md:grid-cols-4 lg:gap-12">
          <div className="md:col-span-1">
            <Link
              href="/#home"
              onClick={handleLogoClick}
              className="group flex w-fit cursor-pointer items-center gap-3"
              aria-label="NBBS Home"
            >
              <Image
                src="/nbbs-logo.webp"
                alt="NBBS Logo"
                width={50}
                height={30}
                className="h-auto w-[clamp(2.5rem,2rem+1.5vw,3.125rem)] object-contain brightness-0 invert"
                priority
              />
              <span className="text-headline-md font-display font-bold tracking-tight text-white">
                NBBS
              </span>
            </Link>

            <p className="text-body-md mt-5 max-w-xs text-slate-300">
              Serving the ambitious SMB community in Tier-2 India. Diagnosis
              before prescription. Results over hype.
            </p>

            <p className="text-body-sm mt-6 text-slate-400">
              Nashik, Maharashtra · connect@nbbs.in
            </p>
          </div>

          <div className="md:col-span-3">
            <div className="grid grid-cols-2 gap-10 sm:grid-cols-3">
              <div>
                <h3 className="text-eyebrow mb-5 text-white">Solutions</h3>
                <div className="flex flex-col gap-4">
                  <Link href="/#service-01" className={linkClass}>
                    Business Clarity Workshop
                  </Link>
                  <Link href="/#service-02" className={linkClass}>
                    Business OPD™
                  </Link>
                  <Link href="/#service-03" className={linkClass}>
                    Incentiwise
                  </Link>
                </div>
              </div>

              <div>
                <h3 className="text-eyebrow mb-5 text-white">Company</h3>
                <div className="flex flex-col gap-4">
                  <Link
                    href="#about"
                    onClick={(e) => {
                      e.preventDefault();
                      document.getElementById("about")?.scrollIntoView({
                        behavior: "smooth",
                        block: "start",
                      });
                    }}
                    className={linkClass}
                  >
                    About Us
                  </Link>
                  <Link
                    href="#vision"
                    onClick={(e) => {
                      e.preventDefault();
                      document.getElementById("vision")?.scrollIntoView({
                        behavior: "smooth",
                        block: "start",
                      });
                    }}
                    className={linkClass}
                  >
                    Our Vision
                  </Link>
                  <Link
                    href="#mission"
                    onClick={(e) => {
                      e.preventDefault();
                      document.getElementById("mission")?.scrollIntoView({
                        behavior: "smooth",
                        block: "start",
                      });
                    }}
                    className={linkClass}
                  >
                    Our Mission
                  </Link>
                  <Link
                    href="#services"
                    onClick={(e) => {
                      e.preventDefault();
                      document.getElementById("services")?.scrollIntoView({
                        behavior: "smooth",
                        block: "start",
                      });
                    }}
                    className={linkClass}
                  >
                    Services
                  </Link>
                </div>
              </div>

              <div className="flex flex-col gap-4">
                <h3 className="text-eyebrow text-white">Contact</h3>
                <a
                  href={`https://www.google.com/maps/place/NB+Business+Solutions/@20.00577,73.768974,3a,75y,89.18h,90t/data=!3m7!1e1!3m5!1sfTWCNlzWH7mpZaUUCa-u8A!2e0!6shttps:%2F%2Fstreetviewpixels-pa.googleapis.com%2Fv1%2Fthumbnail%3Fcb_client%3Dmaps_sv.tactile%26w%3D900%26h%3D600%26pitch%3D0%26panoid%3DfTWCNlzWH7mpZaUUCa-u8A%26yaw%3D89.17774!7i16384!8i8192!4m10!1m2!2m1!1snbbs+solution!3m6!1s0x3bddebece0e0085b:0x723bcd9f9798b5c3!8m2!3d20.0057655!4d73.7690401!15sCg1uYmJzIHNvbHV0aW9uWg8iDW5iYnMgc29sdXRpb26SAR5idXNpbmVzc19tYW5hZ2VtZW50X2NvbnN1bHRhbnTgAQA!16s%2Fg%2F11wpm7fbps?entry=ttu&g_ep=EgoyMDI2MDgyNi4wIKXMDSoASAFQAw%3D%3D=${encodeURIComponent(
                    footerData.contact.location,
                  )}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className={linkClass}
                >
                  {footerData.contact.location}
                </a>
                <a
                  href={`tel:${footerData.contact.phone}`}
                  className={linkClass}
                >
                  {footerData.contact.phone}
                </a>
                <a
                  href={`mailto:${footerData.contact.email}`}
                  className={linkClass}
                >
                  {footerData.contact.email}
                </a>
              </div>
            </div>
          </div>
        </div>

        <div className="mt-12 border-t border-white/10 pt-6">
          <div className="text-body-sm flex flex-col gap-3 text-slate-400 sm:flex-row sm:items-center sm:justify-between">
            <span>© 2026 NB Business Solutions, Nashik.</span>
            <div className="flex flex-wrap gap-3">
              <Link href="#" className="transition-colors hover:text-white">
                Disclaimer
              </Link>
              <Link href="#" className="transition-colors hover:text-white">
                IP
              </Link>
              <Link href="#" className="transition-colors hover:text-white">
                Cookies
              </Link>
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
}
